...
class AdminHubPage extends StatelessWidget {
  ...
  Future<void> _ensureQaRoleDocs(BuildContext context, bool isArabic) async {
    ...
    final displayName = _displayNameForQa(user);

    final clinicianRef = db.collection('clinicians').doc(uid);
    final clientRef = db.collection('clients').doc(uid);
    final centerRef = db.collection('centers').doc(uid);

    final clinicianSnap = await clinicianRef.get();
    ...
    batch.set(
      clinicianRef,
      {
        ...
        'blockedBy': '',
        'blockReason': '',
        if (!clinicianSnap.exists) 'createdAt': FieldValue.serverTimestamp(),
    ...
    batch.set(
      clientRef,
      {
        ...
        'blockedBy': '',
        'blockReason': '',
        if (!clientSnap.exists) 'createdAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
      },
      SetOptions(merge: true),
    );
    batch.set(
      centerRef,
      {
        ...
        'galleryImages': centerSnap.data()?['galleryImages'] ?? <String>[],
        'documentItems':
            centerSnap.data()?['documentItems'] ?? <Map<String, dynamic>>[],
        'isBlocked': false,
        'blockedAt': null,
        'blockedBy': '',
        'blockReason': '',
        if (!centerSnap.exists) 'createdAt': FieldValue.serverTimestamp(),
        'updatedAt': FieldValue.serverTimestamp(),
      },
      SetOptions(merge: true),
    );
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          isArabic
              ? 'تم تجهيز بيانات الأدوار لاختبار الحساب.'
              : 'QA role documents are ready for this account.',
        ),
      ),
    );
  }

  Future<void> _openQaRole(
    BuildContext context, {
    required String route,
    required String roleOverride,
    required bool isArabic,
  }) async {
    await _ensureQaRoleDocs(context, isArabic);
    if (!context.mounted) return;
    Navigator.of(context).pushNamed(
      route,
      arguments: {
        'qaRoleOverride': roleOverride,
      },
    );
  }
            ...
            builder: (context, constraints) {
              ...
              return ListView(
                ...
                children: [
                  ...
                  if (_isQaMultiRoleAdmin()) ...[
                    const SizedBox(height: AppSpacing.lg),
                    _QaRoleAccessSection(
                      isArabic: isArabic,
                      onPrepare: () => _ensureQaRoleDocs(context, isArabic),
                      onOpenClinician: () => _openQaRole(
                        context,
                        route: Routes.clinicianOperations,
                        roleOverride: 'clinician',
                        isArabic: isArabic,
                      ),
                      onOpenClient: () => _openQaRole(
                        context,
                        route: Routes.clientDashboard,
                        roleOverride: 'client',
                        isArabic: isArabic,
                      ),
                      onOpenCenter: () => _openQaRole(
                        context,
...
class _QaRoleAccessSection extends StatelessWidget {
  const _QaRoleAccessSection({
    required this.isArabic,
    required this.onPrepare,
    required this.onOpenClinician,
  ...
  final Future<void> Function() onOpenClinician;
    ...
    return AppSurfaceCard(
      child: Column(
        ...
        children: [
          Text(
            isArabic
                ? 'اختبار الأدوار لحسابات الإدارة'
                : 'Admin QA role access',
          ...
          Text(
            isArabic
                ? 'هذه الأدوات مخصصة لحسابي Admin 1 و Admin 3 لتجهيز بيانات الأخصائي والعميل والمركز وفتح الصفحات مباشرة دون كسر أولوية دخول الإدارة.'
                : 'These tools are for Admin 1 and Admin 3 to prepare clinician/client/center docs and open those pages directly without changing admin-first landing.',
            textAlign: isArabic ? TextAlign.right : TextAlign.left,
          ),
          const SizedBox(height: AppSpacing.md),
          Wrap(
            ...
            children: [
              FilledButton.icon(
                ...
                label: Text(
                  isArabic ? 'تجهيز بيانات الأدوار' : 'Prepare role docs',
                ),
              ),
              OutlinedButton.icon(
                onPressed: onOpenClinician,
                icon: const Icon(Icons.medical_services_outlined),
                label: Text(
                  isArabic ? 'فتح وضع الأخصائي' : 'Open clinician view',
                ),
              ),
              OutlinedButton.icon(
                ...
                label: Text(
                  isArabic ? 'فتح وضع العميل' : 'Open client view',
                ),
              ),
              OutlinedButton.icon(
                ...
                label: Text(
                  isArabic ? 'فتح وضع المركز' : 'Open center view',
                ),
              ),
            ],
          ),
