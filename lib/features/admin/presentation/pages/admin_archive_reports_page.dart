...
class _AdminArchiveReportsPageState extends State<AdminArchiveReportsPage> {
  ...
  String _assistantAccountingCsv(
    ...
    final isArabic = _isArabic(context);
    final rows = _assistantAccountingRows(filtered);
    final headers = [
      isArabic ? 'ID' : 'ID',
  ...
  List<String> _assistantLines(
    ...
    if (_assistantPreset != 'none') {
      switch (_assistantPreset) {
        case 'monthly':
          return _assistantMonthlyLines(context, filtered);
        case 'section_compare':
          return _assistantSectionCompareLines(context, filtered);
        case 'clinician_performance':
          return _assistantClinicianPerformanceLines(context, filtered);
        case 'center_ops':
          return _assistantCenterOpsLines(context, filtered);
      }
    }
    switch (_assistantMode) {
      case 'stats':
        return _assistantStatsLines(context, filtered);
      case 'analysis':
        return _assistantAnalysisLines(context, filtered);
      case 'accounting':
        return _assistantAccountingLines(context, filtered);
      default:
        return _assistantSummaryLines(context, filtered);
    }
  }
  ...
  Widget _assistantOutput(
    ...
    final lines = _assistantLines(context, filtered);
    ...
    final isArabic = _isArabic(context);
    final rows = _assistantAccountingRows(filtered);
    final csv = _assistantAccountingCsv(context, filtered);
  ...
  List<String> _fullReportLines(
    ...
    final lines = <String>[
      ...
      if ((data['assignedClinicianName'] ?? data['clinicianName'] ?? '')
          ...
          .trim()
          .isNotEmpty)
        '${isArabic ? 'الأخصائي' : 'Clinician'}: ${(data['assignedClinicianName'] ?? data['clinicianName'] ?? '').toString()}',
      ...
      if ((data['sessionDateText'] ?? '').toString().trim().isNotEmpty)
        '${isArabic ? 'موعد الجلسة' : 'Session date'}: ${(data['sessionDateText'] ?? '').toString()}',
      if ((data['sessionCode'] ?? '').toString().trim().isNotEmpty)
        '${isArabic ? 'كود الجلسة' : 'Session code'}: ${(data['sessionCode'] ?? '').toString()}',
      if ((data['sessionLink'] ?? '').toString().trim().isNotEmpty)
        '${isArabic ? 'رابط الجلسة' : 'Session link'}: ${(data['sessionLink'] ?? '').toString()}',
      ...
      ...ratings.map((rating) {
        ...
        final percentage = ((rating['percentageScore'] ?? 0) as num)
            .toDouble()
            .toStringAsFixed(1);
        return '${reviewerType == 'clinician' ? (isArabic ? 'تقييم الأخصائي' : 'Clinician review') : (isArabic ? 'تقييم العميل' : 'Client review')}: $percentage%';
      }),
    ];
  ...
  List<String> _financialPrintLines(
    ...
    final clinicianId =
        (data['assignedClinicianId'] ?? data['clinicianId'] ?? '').toString();
    ...
    return [
      ...
      if (clinicianId.trim().isNotEmpty)
        '${isArabic ? 'معرف الأخصائي' : 'Clinician ID'}: $clinicianId',
      ...
      child: AppSurfaceCard(
        ...
        child: ExpansionTile(
          ...
          children: [
            ...
            _sectionCard(
              context,
              title: isArabic ? 'بيانات الجلسة' : 'Session data',
              children: [
                _infoRow(
                  context,
